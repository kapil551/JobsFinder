...
router.put("/jobs/:id", protect, (req, res) => {
...
// to delete a job
router.delete("/jobs/:id", protect, (req, res) => {
  const user = JSON.parse(req.query.user);
  console.log('user', user);
  if (user._id != "6282b84e22de0cdcf16c17d2") {
    res.status(401).json({
  ...
  User.findOne({ _id: req.params.id })
    .then((userData) => {
      ...
      if (userData.type === "recruiter") {
        Admin.findOne({ userId: userData._id })
          .then((recruiter) => {
            if (recruiter === null) {
              res.status(404).json({
                message: "User does not exist",
...
// update user details
router.put("/user", protect, (req, res) => {
  const user = req.user;
  const data = req.body;
  if (user._id === "6282b84e22de0cdcf16c17d2") {
    Admin.findOne({ userId: user._id })
...
router.post("/jobs/:id/applications", protect, (req, res) => {
  const user = req.body;
  if (user._id === "6282b84e22de0cdcf16c17d2") {
    res.status(401).json({
...
router.get("/jobs/:id/applications", protect, (req, res) => {
  // const user = req.user;
  console.log('req', req);
  // if (user._id != "6282b84e22de0cdcf16c17d2") {
  //   res.status(401).json({
...
router.put("/applications/:id", protect, (req, res) => {
  const user = req.user;
...
router.get("/applicants", protect, (req, res) => {
  const user = JSON.parse(req.query.user);
  // console.log("user", user);
  if (user._id === "6282b84e22de0cdcf16c17d2") {
    let findParams = {
...
router.put("/rating", protect, (req, res) => {
  const user = req.user;
  const data = req.body;
  if (user._id === "6282b84e22de0cdcf16c17d2") {
    // can rate applicant
...
// get personal rating
router.get("/rating", protect, (req, res) => {
  const user = req.user;
  Rating.findOne({
    senderId: user._id,
