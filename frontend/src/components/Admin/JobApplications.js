...
const ApplicationTile = ({user, ...props}) => {
  const classes = useStyles();
  ...
  const getResume = () => {
    if (
      ...
      const address = `${`http://localhost:5000`}${application.jobApplicant.resume}`;
      console.log(address);
      axios(address, {
        method: "GET",
        responseType: "blob",
  ...
  return (
    <Paper className={classes.jobTileOuter} elevation={3}>
      <Grid container>
        ...
        >
          {/* <Avatar
            src={`${"http://localhost:5000"}${application.jobApplicant.profile}`}
        ...
        >
          {applications.length > 0 ? (
            applications.map((obj) => (
              <Grid item>
                {/* {console.log(obj)} */}
                <ApplicationTile application={obj} getData={getData} user={user} />
              </Grid>
            ))
          ) : (
